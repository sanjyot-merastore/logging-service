...
public abstract class BaseBatchedElasticsearchSink : IBatchedLogEventSink
{
  ...
  protected BaseBatchedElasticsearchSink(string elasticsearchUrl, string indexFormat)
  {
  ...
  // This is similar to the individual Emit logic from BaseElasticsearchSink.
  ...
  protected Dictionary<string, object> GetCommonLogFields(LogEvent logEvent)
  {
    ...
    fields["service-name"] = Constants.ServiceName;
  ...
  public abstract Task EmitBatchAsync(IReadOnlyCollection<LogEvent> batch);
}